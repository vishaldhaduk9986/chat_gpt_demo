...
class CodeEditDemo extends StatefulWidget {
  const CodeEditDemo({
    super.key,
  });

  @override
  State<CodeEditDemo> createState() => _CodeEditDemoState();
}

class _CodeEditDemoState extends State<CodeEditDemo> {
    ...
    return IconButton(
      ...
      onPressed: () async {
        if (_textController.text.isNotEmpty) {
          ...
          var newMessage = await API.shared.getCodeEdit(input);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('code-davinci-edit-001'),
      ),
      body: SafeArea(
        child: Padding(
